...
class ServerScanner:
    ...
    def connect_to_host(self, server_proxy):
        ...
        self.start_countdown(1, server_proxy)
    ...
    def start_countdown(self, count, server_proxy):
        ...
        if count > 0:
            ...
            self.finalize_connection(server_proxy)

    def finalize_connection(self, server_proxy):
        """Finaliza a conexão com o servidor e realiza registro e comunicação inicial."""
        try:
            print("Conectando ao servidor...")
            response = server_proxy.ping(client_uri)
            print(f"Resposta do servidor: {response}")
            if response == "pong":
                self.update_status("Conexão com o servidor estabelecida com sucesso!")
                self.connected = True
    ...
    def register_with_server(self):
        """Registra o cliente com o servidor usando um identificador único e proxy."""
        if self.server_proxy:
            try:
                print("Registrando cliente no servidor...")
                # Registra o cliente no servidor com seu proxy
                self.server_proxy.register_client(self.client_id)
                print("Cliente registrado com sucesso.")
            except Pyro5.errors.PyroError as e:
                print(f"Erro ao registrar cliente no servidor: {e}")
            except Exception as e:
                print(f"Erro inesperado: {e}")
        else:
            print("Não é possível registrar o cliente. Proxy do servidor não dispon ível.")
    ...
    def send_player_data(self):
        ...
        try:
            ...
            print("Enviando dados do jogador ao servidor...")
            player_data = {
                "Nome": "Jogador 1",
